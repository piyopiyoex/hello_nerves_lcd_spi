defmodule SampleApp.LcdC.LCD do
  @moduledoc """
  LCD制御モジュール (ILI9486 320x480用)
  """
  ...
  def init(spi, dc, rst) do
    ...
    # MX | BGR
    #    send_data(spi, dc, <<0x48>>)
    #    send_data(spi, dc, <<0x28>>)
    send_data(spi, dc, <<0x88>>)
    #    send_data(spi, dc, <<0xE8>>)
  ...
  def load_image_path() do
    priv_path = :code.priv_dir(:hello_kiosk01) |> to_string()
